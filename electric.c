...
OK. so there are three mistakes.   Lets start with the simple one:  carg returns [-pi, pi] you want zero to one, so
...
The reason for 1-GiveTurn(g) is because the gradient should point uphill, not downhill.  Thus a minus sign.  

Next the gradient is a vector (a complex number), but in your code it was declared as a real!!  Fix that:
...
Next, the gradient formula in the code was not quite right; you wrote cabs(Z*dZ) but that's wrong; there is no cabs. Also Z should be Zbar, the complex conjugate of Z.   So it should have been conj(Z)*dZ
...
You would have caught these mistakes if you tried to compute the gradient yourself ... you should do it, its not that hard.

Attached is the final code, and the image.  Your new colormap is different and ugly-ish. The line between red-and-yellow is much much worse than before.  the red-yellow discontinuity is ... confusing, annoying. .. to me, at least.
...
--linas
 ...
 formated with emacs
   ...
   convert landing.ppm landing.png  
...
const int iWidth  = 5000; 
const int iHeight = 5000;
...
int ComputeAndSavePixelColor(int iX, int iY){
  ...
  GiveLinasColor(gm , k,  data); // color depends on the  gradients modulus 
